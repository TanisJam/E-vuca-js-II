...
let fernando = new Usuario('Pedro', 'Evuca');

console.log(juan.introduccion()); // Mi nombre es Juan, mi usuario de Github es juan.perez.
console.log(riley.introduccion()); // Mi nombre es Pedro, mi usuario de Github es Evuca.
...
> Object.assign(obj, {nombre:'Cesar', apellido:'Perez'})
...
> var Emi = new Persona('Cesar', 'Perez', 'Buenos Aires');

> Emi.saludar()
< 'Soy Cesar de Buenos Aires'
```

Ahora todo Alumno de Evuca antes de Alumno es una Persona, asique podríamos decir que un Alumno hereda las propiedades de ser Persona.
...
> var nahuel = new Alumno('Cesar', 'Perez', 'Buenos Aires', 'Web Full Stack')
...
> nahuel.saludar()
< Uncaught TypeError: nahuel.saludar is not a 'function'
// que paso?
```
...
> var Pedro = new Alumno('Pedro','Evuca','Montevideo','Bootcamp')

> Pedro.saludar()
< 'Soy Pedro de Montevideo'
```
